...
#user = sys.argv[3]
# for k in arg:
#     while(arg[k] != ">" and arg[0] == "<"):
#         command.append(arg[k])
...
command = arg[0: result]

result2 = result +1
code = arg[result2:  ]
# aid = "help"

# f2=open("receptacles/recp.txt","w")
# f2.write(command)
# f2.close()

# f=open("receptacles/code.txt","w")
# f.write(code)
# f.close()

# f1 = open("receptacles/all.txt","w")
# f1.write(arg)
# f1.close()
...
#         os.close(3)
#         os.dup2(0, 3)                  #reserve fd3                  #jackson modification
#         command_pipe = os.pipe()
#         code_pipe = os.pipe()
#         if os.fork() != 0:
#             # parent process
#             os.close(3)
#             os.dup2(command_pipe[0], 3)
#             os.close(command_pipe[0])
#             os.close(command_pipe[1])

#             os.close(0)
#             os.dup(code_pipe[0])
#             os.close(code_pipe[0])
#             os.close(code_pipe[1])
#             os.execl(shell, 'CPET', '-c', receptCmd + ' ' + tmpdir)
#         else:
#             # child process
#             os.close(1)
#             os.dup(command_pipe[1])
#             os.close(command_pipe[0])
#             os.close(command_pipe[1])
#             print >> sys.stdout, command + "\n"
            
#             code_descriptor_number = os.dup(code_pipe[1])
#             code_file_object = os.fdopen(code_descriptor_number, "w")
#             os.close(code_pipe[0])
#             os.close(code_pipe[1])
#             print >> code_file_object, code + "\n"
...
try:
    ...
    # regexp2 = re.compile(r"^([a-zA-Z0-9_-]+)")
    # regexp = re.compile(r"^<([a-zA-Z0-9_-]+).*> (?:\n|\r\n?)([a-zA-Z0-9_-]+)$", re.MULTILINE)
    # print("what?"+regexp)
# r"^<([a-zA-Z0-9_-]+).*> (?:\n|\r\n?)([a-zA-Z0-9_-]+)$"
    # match2 = regexp2.match(code)
